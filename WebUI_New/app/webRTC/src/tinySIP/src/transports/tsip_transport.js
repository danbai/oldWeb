...
tsip_transport.prototype.send = function (s_branch, o_message, s_dest_ip, i_dest_port) {
    ...
    * CANCEL will have the same Via and Contact headers as the request it cancel */
    ...
    if (divCallOptions.style.display !== 'inline-block' && o_message.o_hdr_To.o_uri.s_user_name === txtPrivateIdentity.value && o_message.line.response.i_status_code === 200) {
        divCallOptions.style.display = 'inline-block';
        window.jQuery("#btnHoldResume")[0].disabled = false;
        window.jQuery("#btnTransfer")[0].disabled = false;
    }

    sRemoteCalleeName = o_message.o_hdr_From.s_display_name;
