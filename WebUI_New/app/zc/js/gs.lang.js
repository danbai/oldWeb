...
(function($, document, undefined) {
    ...
    var current_locale = window.currentLocale;
    var _init = function() {
        // if (inited) {
        //     return;
        // }
        ...
        // }
        locales[window.currentLocale] = window.currentLocaleData
        ...
        if (!isDoc) {
            ...
            if (req && req.indexOf("^") !== -1) {
                var localeArr = req.split("^"),
                    localeArrStr = locales[current_locale][localeArr[0]] || localeArr[0];

                for (var i = 1; i < localeArr.length; i++) {
                    if (/[\']/.test(localeArr[i])) {
                        if (localeArrStr.indexOf("{0}") !== -1) {
                            localeArrStr = localeArrStr.format(localeArr[i].replace(/(\')/g, ""));
                        } else {
                            localeArrStr += " " + localeArr[i].replace(/(\')/g, "");
                        }
                    } else if (localeArrStr.indexOf("{0}") !== -1) {
                        var str = locales[current_locale][localeArr[i]] || localeArr[i]
                        localeArrStr = localeArrStr.format(str);
                    } else {
                ...
                if (customLocale !== null &&
                    ...
                    if (result == undefined && current_locale != default_locale) {
                        result = locales[default_locale][req];
                    }
                }
            ...
            //     var required = $(this).attr("required");
                ...
                _buildLabel(req, isDoc, isReload);
            // });
