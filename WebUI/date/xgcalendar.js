...
(function($) {
    ...
    var sNowDate = new Date(),
        sNowTime = $(".sysTime", top.frames["frameContainer"].document).text(),
        sYear = sNowTime.slice(0, 4),
        sMonth = parseInt(sNowTime.slice(5, 7), 10) - 1,
        sDate = parseInt(sNowTime.slice(8, 10), 10),
        sHour = parseInt(sNowTime.slice(11, 13), 10),
        sMin = parseInt(sNowTime.slice(14, 16), 10);

    sNowDate.setFullYear(sYear, sMonth, sDate);
    sNowDate.setHours(sHour, sMin);
    ...
    $.fn.bcalendar = function(option) {
        var def = {
            view: "week",
            //默认是周视图day,week,month
            weekstartday: 0,
            //默认星期一开始
            theme: 0,
            //默认使用第一套主题
            height: false,
            //视图的高度，如果不设置则默认获取所在页面的高度
            url: "",
            //请求数据的Url
            eventItems: [],
            //日程数据，可通过此参数设置初始化数据
            method: "POST",
            //异步提交数据的方式，默认为POST建议不要修改。
            showday: new Date(),
            //显示日期，默认为当天
            ...
            //当周视图切换到日视图，因为在转换在内部完成，所以公开一个入口可得到该行为
            quickAddHandler: false,
            //快速添加的拦截函数，该参数设置后quickAddUrl参数的设置将被忽略
            quickAddUrl: "",
            //快速添加日程响应的 Url 地址
            quickUpdateUrl: "",
            //拖拽更新时响应的 Url 地址
            ...
            //快速删除日程时响应的Urk 地址
            autoload: true,
            //自动加载，如果eventItems参数没有配置，可启用该参数，默认第一次展现时
            readonly: false,
            //是否只读，某些情况下，可设置整个
            extParam: [],
            //额外参数，在所以异步请求中，都会附加的额外参数，可配置其他扩展的查询条件
            enableDrag: false,
            //是否可拖拽
            loadDateR: [],
            //记录已加载过的日程的时间段
            timeFormat: " HH:mm",
            //t表示上午下午标识,h 表示12小时制的小时，H表示24小时制的小时,m表示分钟
        ...
        function render() {
            ...
            case "month":
                ...
                break;
            default:
                console.log(i18n.xgcalendar.no_implement);
                break;
            }
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            for (var i = 0; i < dayarrs.length; i++) {
                ...
                if (dateFormat.call(dayarrs[i].date, "yyyyMMdd") == dateFormat.call(sNowDate, "yyyyMMdd")) {
        ...
        function BuildDayScollEventContainer(ht, dayarrs, events) {
            ...
            for (var i = 0; i < l; i++) {
                ...
                var istoday = dateFormat.call(dayarrs[i].date, "yyyyMMdd") == dateFormat.call(sNowDate, "yyyyMMdd");
                // Today
                if (istoday) {
        ...
        function BuilderMonthBody(htb, showday, startday, events, bodyHeight) {
            ...
            for (var j = 0; j < rc; j++) {
                ...
                for (var i = 0; i < 7; i++) {
                    ...
                    if (dateFormat.call(day, "yyyyMMdd") == dateFormat.call(sNowDate, "yyyyMMdd")) {
                    ...
                    if (e) { //如果存在
                        ...
                        if (!bs && j == (sc - 1) && z[h] < y[h]) {
                            ...
                            $.extend(tempdata, {
                                ...
                                click: "javascript:console.log('more event');"
                            });
...
/*if (option.onBeforeRequestData && $.isFunction(option.onBeforeRequestData)) {
                ...
                /*var sNow = $(".sysTime", top.frames["frameContainer"].document).text(),
                    sZone = sNow.split('UTC')[1],
                    nZonePositive = sZone.slice(0, 1),
                    nZone = parseInt(sZone.slice(1, 3)),
                    zone = nZonePositive === '+' ? +nZone : -nZone;*/
                ...
                $.ajax({
                    ...
                    //data: param,
                    dataType: "json",
                    // fixed jquery 1.4 not support Ms Date Json Format /Date(@Tickets)/
                    //dataType: "json",
                    ...
                    success: function(datastr) {
                        var sNow = $(".sysTime", top.frames["frameContainer"].document).text(),
                            sZone = sNow.split('UTC')[1],
                            nZonePositive = sZone.slice(0, 1),
                            nZone = parseInt(sZone.slice(1, 3), 10),
                            zone = nZonePositive === '+' ? +nZone : -nZone,
                            nTrueZone = new Date().getTimezoneOffset() / 60 * -1,
                            aData = datastr.response.events,
                            subZone = (nTrueZone - zone) * 60 * 60 * 1000,
                            //subZone = 5 * 60 * 60 * 1000,
                            ele, startTime, endTime;

                        for (var i = 0; i < aData.length; i++) {
                            ele = aData[i];
                            startTime = Number(ele[2].slice(6, -2)) - subZone;
                            endTime = Number(ele[3].slice(6, -2)) - subZone;

                            ele[2] = '/Date(' + startTime + ')/';
                            ele[3] = '/Date(' + endTime + ')/';
                        }

                        datastr = JSON.stringify(datastr.response).replace(/"\/(Date\([0-9-]+\))\/"/gi, 'new $1');
                    ...
                    error: function(data) {
                        /*try {
                            if (option.onRequestDataError) {
                                ...
                                console.log(i18n.xgcalendar.get_data_exception);
                            }
                            if (option.onAfterRequestData && $.isFunction(option.onAfterRequestData)) {
                                option.onAfterRequestData(1);
                            }
                ...
                console.log("url" + i18n.xgcalendar.i_undefined);
            }
        }
...
/*else {
        ...
        function dayshow(e, data) {
            if (data == undefined) {
                data = getdata($(this)); //bookid数据
            }
            if (data != null) {
                //if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                var csbuddle = '<div id="bbit-cs-buddle" style="z-index: 10000; width: 400px;visibility:hidden;" class="bubble"><table class="bubble-table" cellSpacing="0" cellPadding="0"><tbody><tr style="border-bottom: 1px solid #ddd;"><td class="bubble-cell-side"><div id="tl1" class="bubble-corner"><div class="bubble-sprite bubble-tl">Event</div></div><td class="bubble-cell-main"><div class="bubble-top"></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div>  <tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div><div class="cb-root"><table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><div id="bbit-cs-what" title="' + i18n.xgcalendar.click_to_detail + '" class="textbox-fill-div lk" style="cursor:pointer;"></div></div></div></td></tr><tr><td class=cb-value><div id="bbit-cs-buddle-timeshow"></div></td></tr></tbody></table><div class="bbit-cs-split"><input id="bbit-cs-id" type="hidden" value=""/><button id="bbit-cs-delete" class="lk btn btn-cancel">Delete</button><button id="bbit-cs-editLink" class="lk btn btn-update">Edit Details</button></div></div></div></div></tbody></table><div id="bubbleClose2" class="bubble-closebutton"></div><div id="prong1" class="prong"><div class=bubble-sprite></div></div></div>';
                ...
                if (bud.length == 0) {
                    ...
                    calbutton.click(function() {
                        ...
                        if (option.DeleteCmdhandler && $.isFunction(option.DeleteCmdhandler)) {
                            option.DeleteCmdhandler.call(this, data, function() {
...
/*$("#bbit-cs-what").click(function(e) {
                            if (!option.ViewCmdhandler) {
                                console.log("ViewCmdhandler" + i18n.xgcalendar.i_undefined);
                            }
                            else {
                    ...
                    lbtn.click(function(e) {
                        if (!option.EditCmdhandler) {
                            console.log("EditCmdhandler" + i18n.xgcalendar.i_undefined);
                        } else {
                            if (option.EditCmdhandler && $.isFunction(option.EditCmdhandler)) {
                                option.EditCmdhandler.call(this, $("#bbit-cs-buddle").data("cdata"), function() {
                                    $("#xgcalendarp").BCalReload();
                                });
                            }
                        }
...
/*else {
                ...
                console.log(i18n.xgcalendar.data_format_error);
            }
            return false;
        }
...
/*if ((!option.quickAddHandler && option.quickAddUrl == "") || option.readonly) {
            ...
            if (buddle.length == 0) {
                ...
                calbutton.click(function(e) {
                    ...
                    var f = /^[^\$\<\>]+$/.test(what);
                    if (!f) {
                        console.log(i18n.xgcalendar.invalid_title);
                        $("#bbit-cal-what").focus();
                        option.isloading = false;
                        return false;
                    }
            ...
            /********************************* 拖拽新增会议 *****************************************************/
            var nCalStartYear = start.getFullYear(),
                nCalStartMonth = addZero(start.getMonth() + 1),
                nCalStartDate = addZero(start.getDate()),
                nCalStartHour = addZero(start.getHours()),
                nCalStartMin = addZero(start.getMinutes()),
                nCalEndHour = addZero(end.getHours()),
                nCalEndMinute = addZero(end.getMinutes());

            if (nCalEndHour === '00' && nCalEndMinute === '00') {
                nCalEndHour = '23';
                nCalEndMinute = '59';
            }
                //nMeetLength = nCalEndHour - nCalStartHour,
            var nMeetLength = (nCalEndHour - nCalStartHour) * 60 + parseInt(nCalEndMinute, 10) - parseInt(nCalStartMin, 10),
                sCalStart = nCalStartYear + '-' + nCalStartMonth + '-' + nCalStartDate + ' ' + nCalStartHour + ':' + nCalStartMin + ':' + '00',
                //t = new Date(),
                //sNow = t.getFullYear() + '-' + addZero(t.getMonth() + 1) + '-' + addZero(t.getDate()) + ' ' + addZero(t.getHours()) + ':' + addZero(t.getMinutes()) + ':' + addZero(t.getSeconds());
                sNow = $(".sysTime", top.frames["frameContainer"].document).text().slice(0, 16);

            if (sCalStart < sNow) {
                top.dialog.dialogMessage({
                    type: 'warning',
                    content: $P.lang("LANG3786"),
                    callback: function() {
                        realsedragevent();
                    }
                });
            ...
            /*if ((nCalStartMin === 30 && nCalEndMinute !== 30) || (nCalStartMin !== 30 && nCalEndMinute === 30)) {
                top.dialog.dialogMessage({
                    type: 'warning',
                    content: $P.lang("LANG4318"),
                    callback: function() {
                        realsedragevent();
                    }
                });
            ...
            //传入分钟时长
            bindAdd(true, sCalStart, nMeetLength);
        ...
        function ResizeView() {
            ...
            if (_viewType == "day" || _viewType == "week") {
                ...
                if ($dvwkcontaienr.length == 0 || $dvtec.length == 0) {
                    console.log(i18n.xgcalendar.view_no_ready);
                    return;
                }
...
/*if (option.readonly == false) {
                    $("#mvEventContainer").mousedown(function(e) { dragStart.call(this, "m1", e); return false; });
                     ...
                     $("#mvEventContainer span.monthdayshow").each(function(e){
                        $(this).click(function(e2) {
                            weekormonthtoday.call($(this).parent()[0], e2); 
                        });
                     });
                }*/
            }
        ...
        function dragEnd(e) {
            if (_dragdata) {
                ...
                if (cgh && (cgh.eh > 24 || (cgh.eh === 24 && cgh.em > 0))) {
                    cgh.eh = 24;
                    cgh.em = 0;
                }
    ...
    $.fn.BCalGoToday = function(d, ifMinical) {
        return this.each(function() {
            if (this.bcal) {
                this.bcal.gt(d, ifMinical);
            }
        })
    };
