...
var checkPingTimer;
var sipUnRegisterFlag = true;

var ucm_user_name = $P.cookie("username"),
    sRemoteCalleeName = '',
    oWebrtcInputConf = {};
...
$(function() {
    $P.lang(doc, true);
    topDoc.title = $P.lang("LANG584").format(config.model_info.model_name, $P.lang("LANG4263"));
    initValidator();
...
function checkPhoneNumber(val, ele) {
...
function initValidator() {
    ...
    $P("#form", doc).validate({
        rules: {
            ...
            "txtPhoneNumber": {
                customCallback: [$P.lang("LANG2767"), checkPhoneNumber]
            }
        },
...
function bindButtonEvent() {
    $('div.top_buttons')
        .delegate('#expert_mode', 'click', function(ev) {
            top.dialog.dialogInnerhtml({
                dialogTitle: $P.lang("LANG229"),
                displayPos: "editForm",
                frameSrc: "html/webrtc_modal.html"
            });
            ev.stopPropagation();
            return false;
        });
...
window.onload = function() {
    ...
    function loadWebrtcInputConf() {
        $.ajax({
            url: '../cgi?action=getWebRTCUserLogins&user_name=' + ucm_user_name,
            type: 'GET',
            dataType: 'json',
            async: false,
            ...
            success: function(data) {
                ...
                if (bool) {
                    oWebrtcInputConf = data.response.user_name;
    ...
    function loadEnableWebrtcDefault() {
        $.ajax({
            url: "../cgi?",
            type: "GET",
            dataType: "json",
                    ...
                    if (!!res) {
                        ...
                        if (extType.toLowerCase() !== 'sip') {
                            top.dialog.dialogMessage({
                                type: 'warning',
                                content: $P.lang("LANG4450")
                            });
                        } else {
                            $.ajax({
                                type: "GET",
                                url: '../cgi?action=getSIPAccount&extension=' + ucm_user_name,
                                async: false,
                                success: function(data) {
                                    var bool = UCMGUI.errorHandler(data);

                                    if (bool) {
                                        var res = data.response.extension,
                                        ...
                                        if (enable_webrtc === 'yes') {
                                            ...
                                            if (txtPrivateIdentity.value === '' || txtWebsocketServerUrl.value === '') {
                                                $.ajax({
                                                    type: "GET",
                                                    ...
                                                    success: function(data) {
                                                        ...
                                                        if (bool) {
                                                            var res = data.response.webrtc_http_settings,
                                                            ...
                                                            txtDisplayName.value = txtPrivateIdentity.value = ucm_user_name;
                                                            txtPublicIdentity.value = 'sip:' + ucm_user_name + '@' + bindaddr;
                                                            txtPassword.value = secret;
                                                            txtWebsocketServerUrl.value = 'ws://' + bindaddr + ':' + bindport + '/ws';
                                                        }
                                                    }
                                                });
                                            }
...
function loadCallOptions() {
    //if (window.localStorage) {
        ...
        bDisableVideo = (oWebrtcInputConf.disable_video === "yes");
...
/*function loadCredentials() {
    if (window.localStorage) {
        ...
        if (!txtWebsocketServerUrl.disabled) {
            if ((s_value = window.localStorage.getItem('org.doubango.identity.websocket_server_url'))) txtWebsocketServerUrl.value = s_value;
        }
...
function saveCredentials() {
    var action = {
        ...
        'register_account_name': $('#txtDisplayName').val(),
        'register_extension': $('#txtPrivateIdentity').val(),
        'register_public_id': $('#txtPublicIdentity').val(),
        'register_password': $('#txtPassword').val(),
        'register_server_url': $('#txtWebsocketServerUrl').val(),
        'dial_number': $('#txtPhoneNumber').val()
    };
    ...
    /*if (window.localStorage) {
        ...
        window.localStorage.setItem('org.doubango.identity.websocket_server_url', txtWebsocketServerUrl.value);
...
function sipCall(s_type) {
    if (txtPhoneNumber.value === '') {
        ...
        return;
    } else if (!txtPhoneNumber.value.match(/^[0-9#*.@:a-zA-Z]{0,64}$/)) {
        txtCallStatus.innerHTML = '<i>Invalid format</i>';
        return;
    }
    ...
    if (oSipStack && !oSipSessionCall && !tsk_string_is_null_or_empty(txtPhoneNumber.value)) {
        ...
        //if (window.localStorage) {
            oConfigCall.bandwidth = tsk_string_to_object(oWebrtcInputConf.max_bandwidth); // already defined at stack-level but redifined to use latest values
            oConfigCall.video_size = tsk_string_to_object(oWebrtcInputConf.video_size); // already defined at stack-level but redifined to use latest values
        //}
...
function sipHangUp() {
    window.clearInterval(checkPingTimer);
    if (oSipSessionCall) {
...
function uiBtnCallSetText(s_text) {
    switch (s_text) {
        case "Call":
            {
                var bDisableCallBtnOptions = false;//(window.localStorage && window.localStorage.getItem('org.doubango.expert.disable_callbtn_options') == "true");
