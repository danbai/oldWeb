                ...
                $.extend(self, {
                    ...
                    reposition: function(event, effect) {
                        ...
                        // Check if mouse was the target
                            ...
                            if (event.view)
                                win = event.view;
                            else if (event.target && event.target.document.parentWindow) {
                                win = event.target.document.parentWindow;
                            }
                            ...
                            event = MOUSE && MOUSE.pageX && (adjust.mouse || !event || !event.pageX) ? {
                                ...
                                event && event.pageX && event.type === 'mousemove' ? event : !adjust.mouse && cache.origin && cache.origin.pageX && options.show.distance ? cache.origin :
                    ...
                    */
                        ...
                        if (effect === FALSE || !visible || isNaN(position.left) || isNaN(position.top) || target === 'mouse' || !$.isFunction(posOptions.effect)) {
                            if (event && (position.left < event.pageX) && ($.cookie('position') !== 'settings_guide') && ($.cookie('localeDirection') !== 'rtl')) {
                                tooltip.hide();
                            } else {
                                if (position.top < 0) {
                                    var nHeight = tooltip.height(),
                                        oTip = tooltip.find('.qtip-tip');

                                    position.top += nHeight + 30;
                                    oTip.hide();
                                }
            ...
            PLUGINS = QTIP.plugins = {
                ...
                offset: function(elem, container) {
                    var pos = elem.offset(),
                        ...
                        quirks = UCMGUI.config.msie && document.compatMode !== 'CSS1Compat',
                        parent = container,
                        scrolled,
                        coffset, overflow;
            ...
            function Tip(qTip, command) {
                ...
                function parseRadius(corner) {
                    var isTitleTop = elems.titlebar && corner.y === TOP,
                        ...
                        // moz = $.browser.mozilla,
                        moz = UCMGUI.config.mozilla,
                        // prefix = moz ? '-moz-' : $.browser.webkit ? '-webkit-' : '',
                        prefix = moz ? '-moz-' : UCMGUI.config.webkit ? '-webkit-' : '',
                ...
                $.extend(self, {
                    init: function() {
                        ...
                        var enabled = parseCorner() && (hasCanvas || UCMGUI.config.msie);
                    ...
                    update: function(corner, position) {
                        ...
                        // VML (IE Proprietary implementation)
                            ...
                            translate[2] = border && /^(r|b)/i.test(corner.string()) ? UCMGUI.config.ie8 ? 2 : 1 : 0;
