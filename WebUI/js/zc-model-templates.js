    ...
    $('div.top_buttons')
        .delegate('#btnAdd', 'click', function (ev) {
            top.dialog.dialogInnerhtml({
                ...
                displayPos: "editForm",
                frameSrc: "html/zc_modeltemplate_modal.html?mode=add"
            });
...
function prepareTemplates(callback) {
    ...
    ZEROCONFIG.connector.getAllTemplates("model", "").done(function (result) {
        if (result.status == 0) {
...
function createTable(source) {
    ...
    $("#item-list").jqGrid({
        ...
        height: "auto",
        colNames: [
            '<span locale="LANG3461">' + $P.lang('LANG3461') + '</span>',
            ...
            '<span locale="LANG3463">' + $P.lang('LANG3463') + '</span>',
...
function transData(cellvalue, options, rowObject) {
    ...
    switch (options.colModel.name) {
        case 'model':
            var model = BLL.DataCollection.getModel(rowObject.model_id),
                label = "UNKNOWN";
            ...
            context = '<span class="name">' + label + '</span>';
