...
$(function() {
    ...
    topDoc.title = $P.lang("LANG584").format(config.model_info.model_name, $P.lang("LANG3252"));
    ...
    $.ajax({
        url: "../cgi?",
        type: "POST",
        data: "action=getMFCR2Signalingtrace&trace_switch=",
        error: checkFailed,
        success: checkReturn
    });
...
function btnStart_click() {
    ...
    $.ajax({
        url: "../cgi?action=traceMFCR2Signaling&control=start",
        type: "GET",
...
function btnStop_click() {
    ...
    $.ajax({
        url: "../cgi?action=traceMFCR2Signaling&control=stop",
        type: "GET",
        error: function(jqXHR, textStatus, errorThrown) {
            top.dialog.dialogMessage({
                type: 'error',
...
function btnDownload_click() {
    window.open("../cgi?action=downloadFile&type=mfcr2_signaling_trace", '_self');
}

function btnDelete_click() {
    $.ajax({
        url: "../cgi?action=removeFile&type=mfcr2_signaling_trace",
        type: "GET",
        error: function(jqXHR, textStatus, errorThrown) {
            top.dialog.dialogMessage({
                type: 'error',
...
function btnDownload_check_state() {
    $.ajax({
        ...
        dataType: "json",
        async: false,
        url: "../cgi?action=checkFile&type=mfcr2_signaling_trace",
        success: function(data) {
