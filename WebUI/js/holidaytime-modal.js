...
function edit_time_condition_apply_work() {
    ...
    if (mode == "edit") {
        ...
        action["time_conditions_holiday"] = gup.call(window, "condition_index");
    } else if (mode == "add") {
        action["action"] = "addTimeConditionHoliday";
    }

    action["name"] = $('#name').val();
    action["memo"] = $('#memo').val();
        ...
        success: function(data) {
            ...
            if (bool) {
                ...
                var DO_RELOAD = function() {
                    mWindow.$("#holidaytime-list", mWindow.document).trigger('reloadGrid');
...
function holidayNameIsExist() {
    var holidayName = $("#name").val(),
        holidayNameList = mWindow.holidayNameList,
        tmpHolidayNameList = [];

    tmpHolidayNameList = holidayNameList.copy(tmpHolidayNameList);

    if (oldHolidayName) {
        tmpHolidayNameList.remove(oldHolidayName);
    }

    return !UCMGUI.inArray(holidayName, tmpHolidayNameList);
}
    ...
    $P("#form", doc).validate({
        rules: {
            "name": {
                ...
                minlength: 2,
                cidName: true,
                isExist: [$P.lang("LANG2137"), holidayNameIsExist]
            },
            "new_itrl_stime_hour": {
                // required: true,
                digits: true,
                range: [0, 23],
                // LANG184  LANG169
                customCallback: [$P.lang('LANG4024').format($P.lang('LANG169'), $P.lang('LANG184')), check_time] //end time's value cannot small than start time's value.
            },
            "new_itrl_ftime_hour": {
                // required: true,
                digits: true,
                range: [0, 23],
                customCallback: [$P.lang('LANG4024').format($P.lang('LANG169'), $P.lang('LANG184')), check_time]
            },
        ...
        submitHandler: function() {
            ...
            if (check_month_required() && $('#advanced').is(':checked')) {
            ...
            } else if ($('#advanced').is(':checked') == false && check_month_required() && check_days_required()) {
                edit_time_condition_apply_work();
            }
        }
    });
...
function editTimeCondition(index) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (bool) {
                var time_conditions_holiday = data.response.time_conditions_holiday,
                    monthStr = [],
                    dayStr = [],
                    weekStr = [];

                MonthList = time_conditions_holiday.month;
                DayList = time_conditions_holiday.day;
                WeekList = time_conditions_holiday.week_day;
                StartHour = addZero(time_conditions_holiday.start_hour);
                EndHour = addZero(time_conditions_holiday.end_hour);
                StartMin = addZero(time_conditions_holiday.start_min);
                EndMin = addZero(time_conditions_holiday.end_min);
                Advanced = time_conditions_holiday.mode;
                Name = time_conditions_holiday.name;
                Memo = time_conditions_holiday.memo;
                oldHolidayName = Name;
