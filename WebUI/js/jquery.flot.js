					...
					if (hasOwnProperty.call(layerCache, styleKey)) {
						...
						for (var key in styleCache) {
							if (hasOwnProperty.call(styleCache, key)) {
								...
								if (info.active) {
									if (!info.rendered) {
										var infoDiv = info.element;
										var infoText = infoDiv.text();
										if (infoDiv.text().contains("LANG")) {
											if (infoText.contains(" ")) {
												var infoArr = infoText.split(" ");
												var locale = infoArr[0] + " '"+infoArr[1]+"'";
												infoDiv.text($P.lang(infoArr[0]).format(infoArr[1])).attr("locale", locale);
											}
											else {
												var transLocale = $P.lang(infoText);
												if (transLocale.contains("{0}")) {
							                        transLocale = transLocale.replace("{0}", "").trim();
							                    }
												infoDiv = infoDiv.text(transLocale).attr("locale", infoText);
											}
										}
										layer.append(infoDiv);
										info.rendered = true;
									}
								} else {
        ...
        function insertLegend() {
            ...
            for (var i = 0; i < entries.length; ++i) {
                ...
                fragments.push(
                    ...
                    '<td class="legendLabel"' + (entry.locale ? (' locale="' + entry.locale + '">') : '>') + entry.label + '</td>'
                );
            }
            ...
            if (options.legend.container != null)
                ...
                if (options.legend.backgroundOpacity != 0.0) {
                    ...
                    $('<div style="position:absolute; width: auto; min-width: 90px; height:' + div.height() + 'px;' + pos +'background-color:' + c + ';"> </div>').prependTo(legend).css('opacity', options.legend.backgroundOpacity);
                }
            }
        }
