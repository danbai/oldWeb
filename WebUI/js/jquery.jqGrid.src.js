...
(function($) {
    ...
    $.extend($.jgrid, {
        ...
        getCellIndex: function(cell) {
            ...
            c = (!c.is('td') && !c.is('th') ? c.parents("td,th") : c)[0];
            if ($.jgrid.msie) {
        ...
        parserPostData: function(data) {
            var data = data,
                postData = {};
            for (var temp in data) {
                if (data[temp] && temp != "_search" && temp != "nd") {
                    if (temp == "rows") {
                        postData["item_num"] = data[temp];
                    } else {
    ...
    $.fn.jqGrid = function(pin) {
        ...
        return this.each(function() {
            ...
            var localeDirection = top.$.cookie('localeDirection');
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                addCell = function(rowId, cell, pos, irow, srvr, rdata) {
                    ...
                    return "<td class=\"wordBreak\" role=\"gridcell\" " + prp + ">" + v + "</td>";
                },
                addMulti = function(rowid, pos, irow, checked) {
                    var v = "<input role=\"checkbox\" type=\"checkbox\"" + " id=\"jqg_" + ts.p.id + "_" + rowid + "\" class=\"cbox styled\" name=\"jqg_" + ts.p.id + "_" + rowid + "\"" + (checked ? "checked=\"checked\"" : "") + "/>",
                ...
                addXmlData = function(xml, t, rcnt, more, adjust) {
                    ...
                    if (gxml && gl) {
                        ...
                        // [AH] Add event support for before inserting row
                        var beforeInsRow = $.isFunction(ts.p.beforeInsertRow),
                        ...
                        while (j < gl) {
                            ...
                            // [AH] Add event support for before inserting row
                            self.triggerHandler("jqGridBeforeInsertRow", [rid, rd, xmlr]);
                            if (beforeInsRow) {
                                ts.p.beforeInsertRow.call(ts, rid, rd, xmlr);
                            }
                ...
                addJSONData = function(data, t, rcnt, more, adjust) {
                    ...
                    while (i < len) {
                        ...
                        var iStartTrTag = rowData.length;
                        // [AH] Add event support for before inserting row
                        self.triggerHandler("jqGridBeforeInsertRow", [idr, rd, cur]);
                        if (beforeInsRow) {
                            ts.p.beforeInsertRow.call(ts, idr, rd, cur);
                        }
                    ...
                    if (!more) {
                        ts.updatepager(false, true, data);
                    }
                    if (locdata) {
                ...
                updatepager = function(rn, dnd, data) {
                    var cp, last, base, from, to, tot, fmt, pgboxes = "",
                        ...
                        tableContainer = $("#gbox_" + $.jgrid.jqID(ts.p.id));
                    ...
                    if (pgboxes) {
                        ...
                        if (ts.p.viewrecords) {
                            if (ts.p.reccount === 0) {
                                ...
                                // Pengcheng Zou Edit.
                                // $(".pageDes", pgboxes).html(ts.p.emptyrecords);

                                var tableWidget = $('#widget_' + $.jgrid.jqID(ts.p.id));

                                if (tableWidget.length > 0) {
                                    tableWidget.show();
                                } else {
                                    var localeArr = ts.p.noData.split(" "),
                                        localeArrStr = $P.lang(localeArr[0]),
                                        transLocale = $P.lang(ts.p.noData),
                                        nameLen = transLocale.length,
                                        resultStr = "";

                                    if (ts.p.noData.indexOf(" ") >= 0) {
                                        for (var i = 1; i < localeArr.length; i++) {
                                            if (/[\']/.test(localeArr[i])) {
                                                if (localeArrStr.indexOf("{0}") >= 0) {
                                                    localeArrStr = localeArrStr.format(localeArr[i].replace(/(\')/g, ""));
                                                } else {
                                                    localeArrStr += " " + localeArr[i].replace(/(\')/g, "");
                                                }
                                            } else if (localeArrStr.indexOf("{0}") >= 0) {
                                                localeArrStr = localeArrStr.format($P.lang(localeArr[i]));
                                            } else {
                                    ...
                                    var noDataTips = '<table cellpadding="0" cellspacing="0" border="0" class="tableWidget" id="' +
                                        'widget_' + $.jgrid.jqID(ts.p.id) + '" style="width: 100%;">' +
                                        '<tbody><tr class="even"><td locale="' + ts.p.noData + '">' + resultStr + '</td></tr></tbody></table>';

                                    tableContainer.before(noDataTips);
                                }
                                ...
                                // Pengcheng Zou Add.
                                var pagerText = $("#pagerText", pgboxes),
                                    noDataTips = $('#widget_' + $.jgrid.jqID(ts.p.id));
                                if (pagerText) {
                                    pagerText.remove();
                                }
                                if (noDataTips) {
                                ...
                                $(".pageDes", pgboxes).prepend($.jgrid.format(ts.p.recordtext, tot, cp, last));
                            }
                        }

                        // Pengcheng Zou modified.
                        // Change "ui-state-disabled" to "ui-state-pager-disabled".
                        if (ts.p.pgbuttons === true) {
                            if (cp <= 0) {
                                cp = last = 0;
                            }
                            if (cp === 1 || cp === 0) {
                                $("#first" + tspg + ", #prev" + tspg).addClass('ui-state-pager-disabled').removeClass('ui-state-hover');
                                if (ts.p.toppager) {
                                    $("#first_t" + tspg_t + ", #prev_t" + tspg_t).addClass('ui-state-pager-disabled').removeClass('ui-state-hover');
                                }
                            } else {
                                $("#first" + tspg + ", #prev" + tspg).removeClass('ui-state-pager-disabled');
                                if (ts.p.toppager) {
                                    $("#first_t" + tspg_t + ", #prev_t" + tspg_t).removeClass('ui-state-pager-disabled');
                                }
                            }
                            if (cp === last || cp === 0) {
                                $("#next" + tspg + ", #last" + tspg).addClass('ui-state-pager-disabled').removeClass('ui-state-pager-hover');
                                if (ts.p.toppager) {
                                    $("#next_t" + tspg_t + ", #last_t" + tspg_t).addClass('ui-state-pager-disabled').removeClass('ui-state-hover');
                                }
                            } else {
                                $("#next" + tspg + ", #last" + tspg).removeClass('ui-state-pager-disabled');
                                if (ts.p.toppager) {
                                    $("#next_t" + tspg_t + ", #last_t" + tspg_t).removeClass('ui-state-pager-disabled');
                                }
                            }
                        }
                    }
                    ...
                    if ($.isFunction(ts.p.gridComplete)) {
                        ts.p.gridComplete.call(ts, data);
                    }
                        ...
                        switch (dt) {
                            ...
                            case "script":
                                $.ajax($.extend({
                                    ...
                                    data: $.isFunction(ts.p.serializeGridData) ? $.jgrid.parserPostData(ts.p.serializeGridData.call(ts, ts.p.postData)) : $.jgrid.parserPostData(ts.p.postData),
                                    success: function(data, st, xhr) {

                                        // Pengcheng Zou Added.
                                        // [AH] Add support for custom error handler
                                        var errorHandler = top.UCMGUI.errorHandler;

                                        if (typeof ts.p.customErrorHandler === "function") {
                                            errorHandler = ts.p.customErrorHandler;
                                        }

                                        // var bool = top.UCMGUI.errorHandler(data);
                                    ...
                                    error: function(xhr, st, err) {
                                        ...
                                        //     content: err,
                                        //     callback: function() {
                                        //         top.UCMGUI.logoutFunction.doLogout();
                                        //     }
                                        // });
                ...
                setHeadCheckBox = function(checked) {
                    ...
                    top.Custom.init(document, $('#cb_' + $.jgrid.jqID(ts.p.id), ts.grid.hDiv)[0]);
                },
                setPager = function(pgid, tp) {
                    ...
                    rgt = pgid + "_right";
                    $("#" + $.jgrid.jqID(pgid))
                        .append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:30px;' role='row'><tbody><tr><td id='" + lft + "' class='pager-left'></td><td id='" + rgt + "' class='pager-right'></td></tr></tbody></table></div>")
                        .attr("dir", dir); //explicit setting
                    if (ts.p.rowList.length > 0) {
                        str = "<div class='view'><span class='spanView' locale='LANG174'>View:</span><div style='display: inline-block; position: relative;'>";
                        str += "<select class='ui-pg-selectbox' role='listbox'>";
                    ...
                    /*if(dir==="rtl") { pgl += str; }*/
                    if (ts.p.pginput === true) {
                        pginp = "<input class='ui-pg-input jump' type='text' size='2' maxlength='4' value='' role='textbox'/><button type='button' locale='LANG120' id='goToPage' class='btn btn-paging'>Go</button>";
                    }
                    if (ts.p.pgbuttons === true) {
                        ...
                        //     po.reverse();
                        // }
                        pgl += "<td id='" + po[0] + "' class='ui-pg-button ui-corner-all'><span locale='LANG121' class='btn btn-paging'>First</span></td>";
                        pgl += "<td id='" + po[1] + "' class='ui-pg-button ui-corner-all'><span locale='LANG122' class='btn btn-paging'>Prev</span></td>";
                        /*pgl += pginp !== "" ? sep+pginp+sep:"";*/
                        pgl += "<td id='" + po[2] + "' class='ui-pg-button ui-corner-all'><span locale='LANG123' class='btn btn-paging'>Next</span></td>";
                        pgl += "<td id='" + po[3] + "' class='ui-pg-button ui-corner-all'><span locale='LANG124' class='btn btn-paging'>Last</span></td>";
                    } else if (pginp !== "") {
                        pgl += pginp;
                    }
                    /*if(dir==="ltr") { pgl += str; }*/
                    ...
                    if (ts.p.viewrecords === true) {
                        $("td#" + pgid + "_" + "left" /*ts.p.recordpos*/ , "#" + pgcnt).append("<div dir='" + dir + "' style='text-align:" + "left" /*ts.p.recordpos*/ + "' class='pageDes'>" + pginp + "</div>");
                    }
                    $("#gbox_" + $.jgrid.jqID(ts.p.id)).prepend(str);
                    $("td#" + pgid + "_" + "right" /*ts.p.pagerpos*/ , "#" + pgcnt).append(pgl);
                    ...
                    if (twd > 0) {
                        ...
                        $("td#" + pgid + "_" + "right" /*ts.p.pagerpos*/ , "#" + pgcnt).width(twd);
                    }
                    ts.p._nvtd = [];
                    ts.p._nvtd[0] = twd ? Math.floor((ts.p.width - twd) / 2) : Math.floor(ts.p.width / 3);
                    ts.p._nvtd[1] = 0;
                    pgl = null;
                    $('.ui-pg-selectbox', "#gbox_" + $.jgrid.jqID(ts.p.id)).bind('change', function() {
                        ...
                        if (ts.p.pager) {
                            $('.ui-pg-selectbox', "#gbox_" + $.jgrid.jqID(ts.p.id)).val(this.value);
                        }
                        if (ts.p.toppager) {
                            $('.ui-pg-selectbox', ts.p.toppager).val(this.value);
                        }
                        populate();
                        return false;
                    }).bind('focus', function() {
                        if (this.className.contains("styled") && $(this).siblings("span").length > 0) {
                            $(this).css("left", $(this).siblings("span")[0].offsetWidth);
                        }
                    });
                    if (ts.p.pgbuttons === true) {
                    ...
                    if (ts.p.pginput === true) {
                        $('input.ui-pg-input', "#" + pgcnt).keypress(function(e) {
                            ...
                            if (key === 13) {
                                ...
                                if (($(this).val() <= 0) || (ts.p.page > ts.p.lastpage)) {
                                    var cb = function() {};
                                    ts.p.page = old_page;
                                    if (top.dialog.container.is(":visible")) {
                                        cb = function () {
                                    ...
                                    top.dialog.dialogMessage({
                                        type: 'warning',
                                        content: top.$.lang("LANG929"),
                                        callback: cb
                                    });
                                    $('input.ui-pg-input', "#" + pgcnt).val('')
                                } else {
                                    populate();
                                }
                                return false;
                        ...
                        $('#goToPage', "#" + pgcnt).click(function(ev) {
                            var old_page = ts.p.page;
                            var pageNum = intNum($('input.ui-pg-input', "#" + pgcnt).val(), 1);
                            ts.p.page = (pageNum > 0) ? pageNum : ts.p.page;
                            if (!clearVals(this.id)) {
                                return false;
                            }
                            if ((pageNum <= 0) || (ts.p.page > ts.p.lastpage)) {
                                var cb = function() {};
                                ts.p.page = old_page;
                                if (top.dialog.container.is(":visible")) {
                                    cb = function () {
                ...
                sortData = function(index, idxcol, reload, sor, obj) {
                    ...
                    if (ts.p.multiSort) {
                        ...
                        // Pengcheng Zou add removeClass method.
                        $(previousSelectedTh).attr("aria-selected", "false").removeClass('ui-th-hover');
                        if (ts.p.frozenColumns) {
                            ts.grid.fhDiv.find("span.ui-grid-ico-sort").addClass('ui-state-disabled');

                            // Pengcheng Zou add removeClass method.
                            ts.grid.fhDiv.find("th").attr("aria-selected", "false").removeClass('ui-th-hover');
                        }
                        $("span.ui-icon-" + ts.p.sortorder, newSelectedTh).removeClass('ui-state-disabled');

                        // Pengcheng Zou add addClass method.
                        $(newSelectedTh).attr("aria-selected", "true").addClass('ui-th-hover');
                        if (!ts.p.viewsortcols[0]) {
                            if (ts.p.lastsort !== idxcol) {
                                if (ts.p.frozenColumns) {
            ...
            if (this.p.multiselect) {
                this.p.colNames.unshift("<input role='checkbox' id='cb_" + this.p.id + "' class='cbox styled' type='checkbox'/>");
            ...
            // Pengcheng Zou Edit.
            // Change "ui-icon-triangle-1-n" to "ui-icon-sort-arrow-up"
            // Change "ui-icon-triangle-1-s" to "ui-icon-sort-arrow-down"
            // Add class "ui-icon-sort-asc" and "ui-icon-sort-desc"
            imgs = "<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc" + iac + " ui-icon-sort-asc ui-state-disabled ui-icon-sort-arrow-up ui-sort-" + dir + "'></span>";
            imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc" + idc + " ui-icon-sort-desc ui-state-disabled ui-icon-sort-arrow-down ui-sort-" + dir + "'></span></span>";
            if (ts.p.multiSort) {
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                ...
                if (ts.p.colModel[i].hasOwnProperty('sortable') && !ts.p.colModel[i].sortable) {
                    thead += "<div id='jqgh_" + ts.p.id + "_" + ts.p.colModel[i].name + "' " + tdc + " style='cursor: default;'>" + ts.p.colNames[i];
                } else {
            ...
            imgs = null;
            $(this).append(thead);
            //jlwang add show s-ico.
            $('thead div[id^="jqgh_"] span[locale]', this).addClass('s-title').each(function() {
                $(this).attr('localetitle', $(this).attr('locale'));
            });
            ...
            if (this.p.multiselect) {
                ...
                $('#cb_' + $.jgrid.jqID(ts.p.id), this).bind('click', function() {
                    ...
                    var allCheckbox = $("input[type='checkbox']", "#gbox_" + $.jgrid.jqID(ts.p.id));
                    var inputChecked = allCheckbox.filter(":checked");
                    var inputDisabled = allCheckbox.filter(":disabled");
                    var inputNotDisabled = allCheckbox.not(":disabled");
                    if (inputChecked.length == (allCheckbox.length - inputDisabled.length)) {
                        this.checked = false;
                    }
                    if (this.checked) {
                    ...
                    if (inputDisabled.length != 0) {
                        inputDisabled.attr("checked", false);
            ...
            $("th", thr).each(function(j) {
                ...
                // Pengcheng Zou Add class "ui-th-hover" and title.
                if (sort) {
                    if (ts.p.multiSort) {
                        if (ts.p.viewsortcols[0]) {
                ...
                if ($(e.target).parents("th>span.ui-jqgrid-resize").length !== 1) {
                    return;
                }
                ...
                //jlwang add to show s-ico.
                if ($(this).find('.s-ico').css('display') !== 'none') {
                    $(this).find('.s-title').css('max-width', $(this).width() - 40);
                    $(this).siblings().find('.s-title').css('max-width', '');
                }
                return false;
            });
            ...
            $(ts).before(grid.hDiv).click(function(e) {
                ...
                if ($(ptr).length === 0 || ptr[0].className.indexOf('ui-state-disabled') > -1 || ($(td, ts).parents("table.ui-jqgrid-btable").attr('id') || '').replace("_frozen", "") !== ts.id) {
                    return this;
                }
        ...
        setSelection: function(selection, onsr, e) {
            return this.each(function() {
                ...
                if (!$t.p.multiselect) {
                    ...
                    var checkboxDiv = $("#jqg_" + $.jgrid.jqID($t.p.id) + "_" + $.jgrid.jqID(pt.id));
                    if (!checkboxDiv.is(":disabled")) {
                        checkboxDiv[$t.p.useProp ? 'prop' : 'attr']("checked", stat);
                    }
                    top.Custom.init(document, checkboxDiv[0]);
                    if (fid) {
                        if (ia === -1) {
                            $("#" + $.jgrid.jqID(selection), "#" + $.jgrid.jqID(fid)).addClass("ui-state-highlight");
                        } else {
                    ...
                    var bool = true;
                    $.each($(":checkbox", "#gbox_" + $.jgrid.jqID($t.p.id)), function(index, item) {
                        if (index > 0) {
                            if (!$(this).is(":checked")) {
                                bool = false;
                            }
                    ...
                    try {
                        $(this.p.colModel).each(function(i) {
                            ...
                            if (dval !== undefined) {
                                ...
                                var titleTxt = $.jgrid.stripHtml(vl);
                                if (titleTxt && titleTxt.replace) {
                                    titleTxt = titleTxt.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
                                }
                                title = this.title ? {
                                    "title": titleTxt
                                } : {};
            ...
            if (rdata) {
                ...
                this.each(function() {
                    ...
                    var k = 0,
                        cna = "",
                        lcdata = {},
                        bir = $.isFunction(t.p.beforeInsertRow) ? true : false,
                    ...
                    while (k < datalen) {
                        ...
                        // [AH] Add before insert row event
                        $(t).triggerHandler("jqGridAfterInsertRow", [rowid, data, data]);
                        if (bir) {
                            t.p.beforeInsertRow.call(t, rowid, data, data);
                        }
                        ...
                        if (gi) {
                            v = "<input role=\"checkbox\" type=\"checkbox\"" + " id=\"jqg_" + t.p.id + "_" + rowid + "\" class=\"cbox styled\"/>";
...
(function($) {
    ...
    $.fn.fmatter.rowactions = function(act) {
        var $tr = $(this).parents("tr.jqgrow"),
            rid = $tr.attr("id"),
            $id = $(this).parents("table.ui-jqgrid-btable").attr('id').replace(/_frozen([^_]*)$/, '$1'),
            $grid = $("#" + $id),
            $t = $grid[0],
            p = $t.p,
                        ...
                        $(ts.rows[i].cells[pos]).bind('click', function() {
                            ...
                            if ($(this).hasClass("sgcollapsed")) {
                                ...
                                if (ts.p.subGridOptions.reloadOnExpand === true || (ts.p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid'))) {
                                    ...
                                    $(tr).after("<tr role='row' class='ui-subgrid'>" + atd + "<td colspan='" + parseInt(ts.p.colNames.length - nhc, 10) + "' class='ui-widget-content subgrid-data'><div id=" + pID + "_" + _id + " class='tablediv'></div></td></tr>");
                    ...
                    $($t.rows[i].cells[expCol])
                        ...
                        .bind("click", function(e) {
                            var target = e.target || e.srcElement,
                                ind2 = $.jgrid.stripPref($t.p.idPrefix, $(target, $t.rows).parents("tr.jqgrow")[0].id),
                                pos = $t.p._index[ind2];
                    ...
                    "drop": function(opts) {
                        return $.extend({
                            accept: function(d) {
                                ...
                                var tid = $(d).parents("table.ui-jqgrid-btable");
